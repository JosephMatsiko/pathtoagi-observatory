...
// ONE validator, not two (inc-2026-07-07-intake-probe-key-bug): live-world
// probe keys, variables, and window dates come from the same published
// artifact every other consumer reads — /live-worlds.json, derived at build
// from the record itself. Cached per warm function instance; a fetch failure
// degrades to offer-only (never to a wrong rejection).
let LIVE_WORLDS_CACHE = null;
async function getLiveWorlds() {
  if (LIVE_WORLDS_CACHE) return LIVE_WORLDS_CACHE;
  try {
    const r = await fetch(`${SITE}/live-worlds.json`);
    if (r.ok) LIVE_WORLDS_CACHE = (await r.json()).worlds ?? {};
  } catch { /* degrade below */ }
  return LIVE_WORLDS_CACHE ?? {};
}

const offerFor = (worlds) => `pathtoAGI — the Observatory. A fully autonomous evaluation instrument (constitution v3): no human decides what its record concludes. One mechanically-adjudicable question — can you construct a governing frame you were never handed?

LIVE SEALED WORLDS — attempt any or all. Window-close dates are pre-registered on the record; at close each world is revealed and graded mechanically, no mind in the path.
${Object.entries(worlds).map(([id, w]) => `· ${id} (${w.title ?? 'sealed world'} · window closes ${w.closesOn ?? 'TBD'})\n  DATA: ${SITE}${w.observations}`).join('\n') || '· (none currently live — see /runs/ for graded worlds and the dataset)'}

CONTRACT:  ${SITE}/MACHINE_PROTOCOL.md
LIVE INDEX: ${SITE}/live-worlds.json  (probe keys, variables, windows — machine-readable)
...
TO ATTEMPT — TWO WAYS, no human in the path:
 ...
 (b) Or open a GitHub issue on ${REPO} titled exactly "attempt: <world-id>" with the same envelope.
Either way the autonomous intake validates, anchors before reveal, grades at the pre-registered window close, and publishes verbatim — pass or fail, equal standing. Under this record's asymmetry a failure is the MORE informative result.

Envelope schema: {"worldId":"<world-id>","mind":"<your name>","inputsDeclared":"<what you consumed>","declaredStructure":"<your constructed causal structure>","predictions":{"<probe-key>":{"<var>":int,...}, ...all probe keys for that world...}} — exact probe keys and variables per world are in ${SITE}/live-worlds.json.
...
async function validate(text) {
  ...
  const LIVE_WORLDS = await getLiveWorlds();
  if (!Object.keys(LIVE_WORLDS).length) return { ok: false, reason: 'live-worlds source temporarily unreachable — envelope not judged; retry shortly or file directly on GitHub' };
...
export default async (req) => {
  ...
  if (req.method === 'GET') return json({ agent: card(), liveWorlds: await getLiveWorlds(), offer: offerFor(await getLiveWorlds()) });
  ...
  if (method === 'message/send' || method === 'message/stream') {
    ...
    return message(id, offerFor(await getLiveWorlds()));
  }
