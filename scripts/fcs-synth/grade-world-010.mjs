#!/usr/bin/env node
// Mechanical grader for world-010 (latent-object construction). Run only after
...
//
// Usage: grade-world-010.mjs <keyfile>
import { createHash, createDecipheriv } from 'node:crypto';
import { readFileSync, writeFileSync, readdirSync, existsSync } from 'node:fs';
import { join } from 'node:path';

const keyFile = process.argv[2];
if (!keyFile) { console.error('usage: grade-world-010.mjs <keyfile>'); process.exit(1); }

const DIR = join(process.cwd(), 'experiments', 'fcs-synth-world-010');
...
const probeKey = (h) => `${h.before.a}-${h.before.b}-${h.before.c}-${h.before.d}_do-${h.do.var}-${h.do.value}`;
...
for (const file of files) {
  ...
  const r = { attempt: file, status: 'scored', parsed: false, probesFullyCorrect: 0, variablesCorrect: 0, variablesTotal: probes.length * 4, predictionsTotal: probes.length, positedLatent: false, notes: [] };
  try {
    ...
    for (const h of probes) {
      ...
      if (!pred) continue;
      let v = 0;
      for (const key2 of ['a', 'b', 'c', 'd']) if (Number(pred[key2]) === h.after[key2]) { v++; r.variablesCorrect++; }
      if (v === 4) r.probesFullyCorrect++;
    }
...
writeFileSync(join(DIR, 'GRADING.json'), JSON.stringify({
  worldId: 'world-010', track: 'F — latent-object construction', gradedAt: new Date().toISOString(), graderVersion: 1, lawHash: gotHash,
  scoring: {
...
writeFileSync(join(DIR, 'REVEAL.md'), `# Reveal — fcs-synth-world-010
