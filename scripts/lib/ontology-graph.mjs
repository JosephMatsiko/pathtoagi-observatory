...
export function buildOntology() {
  ...
  for (const ch of challenges) add('challenge', ch.id ?? slug(ch.title ?? 'challenge'), ch.title ?? ch.id, { href: '/challenges/', state: ch.status });
  ...
  for (const o of outreach) add('outreach', o.id, o.role, { href: '/open-call/', state: o.status });
  ...
  const types = [
    ...
    { id: 'outreach', label: 'Outreach Call', layer: 'interface', href: '/open-call/', accent: 'hsl(212 80% 64%)', description: 'A standing public invitation with a governed status: open, engaged, delivered, or closed.', actions: [A.generate, A.audit, A.publish, A.retire] },
