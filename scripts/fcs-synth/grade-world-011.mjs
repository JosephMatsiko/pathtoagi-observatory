#!/usr/bin/env node
// Mechanical grader for world-011 (interventional replication probe). Run only after
...
//
// Usage: grade-world-011.mjs <keyfile>
import { createHash, createDecipheriv } from 'node:crypto';
import { readFileSync, writeFileSync, readdirSync, existsSync } from 'node:fs';
import { join } from 'node:path';

const keyFile = process.argv[2];
if (!keyFile) { console.error('usage: grade-world-011.mjs <keyfile>'); process.exit(1); }

const DIR = join(process.cwd(), 'experiments', 'fcs-synth-world-011');
...
writeFileSync(join(DIR, 'GRADING.json'), JSON.stringify({
  worldId: 'world-011', track: 'F — interventional replication probe', gradedAt: new Date().toISOString(), graderVersion: 1, lawHash: gotHash,
  scoring: {
...
writeFileSync(join(DIR, 'REVEAL.md'), `# Reveal — fcs-synth-world-011
